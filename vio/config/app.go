...
type Config struct {
	Global struct {
		...
		BaseUrl            string `yaml:"base_url" env:"BASE_URL" env-defualt:"http://0.0.0.0:8118/"`
		SecretCode         string `yaml:"secret_code" env:"SECRET_CODE" env-defualt:""`
		MaxUploadSize      string `yaml:"max_upload_size" env:"MAX_UPLOAD_SIZE" env-default:""`
		FileTreeSplitChars int    `yaml:"file_tree_split_chars" env:"FILE_TREE_SPLIT_CHARS" env-defualt:"3"`
	} `yaml:"global"`
	...
	Images struct {
		StoreOriginals bool   `yaml:"store_originals" env:"IMAGE_STORE_ORIGINALS" env-defualt:"false"`
		...
		AutoConvert    string `yaml:"auto_convert" env:"IMAGE_AUTO_CONVERT" env-defualt:"false"`
		JPEGQuality    int    `yaml:"jpeg_quality" env:"IMAGE_JPEG_QUALITY" env-default:"95"`
		PNGCompression int    `yaml:"png_compression" env:"IMAGE_PNG_COMPRESSION" env-defualt:"0"`
	} `yaml:"images"`
	...
	Videos struct {
		...
		LiveResize     bool   `yaml:"live_resize" env:"VIDEO_LIVE_RESIZE" env-default:"true"`
		AutoConvert    string `yaml:"auto_convert" env:"VIDEO_AUTO_CONVERT" env-defualt:"false"`
		FFmpeg         struct {
			TempDir      string `yaml:"temp_dir" env:"VIDEO_FFMPEG_TEMP_DIR" env-default:"/tmp"`
			Preset       string `yaml:"preset" env:"VIDEO_FFMEPG_PRESET" env-default:"slow"`
			CRF          int    `yaml:"crf" env:"VIDEO_FFMPEG_CRF" env-default:"24"`
			BufferSize   int    `yaml:"buffer_size" env:"VIDEO_FFMPEG_BUFFER_SIZE" env-defualt:"1024000"`
			...
			MovFlags     string `yaml:"mov_flags" env:"VIDEO_FFMPEG_MOV_FLAGS" env-defualt:"+faststart"`
			PixFmt       string `yaml:"pix_fmt" env:"VIDEO_MPEG_PIX_FMT" env-default:"yuv420p"`
		} `yaml:"ffmpeg"`
	} `yaml:"videos"`
